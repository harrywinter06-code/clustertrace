...
## Threat model — what clustertrace is and isn't

clustertrace is a **local-first developer tool**. The threat model assumes:

- It runs on a developer's machine, alongside the code under instrumentation.
- The SQLite store at `~/.clustertrace/traces.db` (or `$CLUSTERTRACE_DB`) contains whatever inputs/outputs you pass to instrumented functions.
...
- **Don't trace functions that handle secrets without filtering** — clustertrace will faithfully record them. `CLUSTERTRACE_MAX_PAYLOAD_BYTES` truncates large I/O but does not redact.
...
- **OpenTelemetry-ingested spans are stored verbatim** — clustertrace does not filter attributes.
...
If you need any of these, you probably want a hosted observability tool (Langfuse Cloud, LangSmith, Helicone) instead of clustertrace.
